...
client.on('message', message => { 
  var whitelisted = "406451228004974603"
...
if(message.content.startsWith(prefix + `autorole`)) {
  ...
  if(!perms) return message.reply(`لا تملك صلاحية تعديل الرتب.`)
  ...
  if(!state.trim().toLowerCase() == 'toggle' || !state.trim().toLowerCase() == 'setrole') return message.reply(`Please type a right state, ${prefix}modlogs toggle/setrole [ROLE NAME]`) 
   ...
   if(!newRole) return message.reply(`${prefix}autorole setrole [ROLE NAME]`)
     if(!message.guild.roles.find(`name`,newRole)) return message.reply(`لا استطيع ايجاد هذه الرتبة.`)
    ar[message.guild.id].role = newRole
     message.channel.send(`**الرتبة التلقائية تم تعديلها الى${newRole}.**`)
   } 
         }
  }
...
if(message.content === prefix + 'info') {
    ...
    if(!perms) return message.reply(`انت لا تمتلك الصلاحيات.`)
